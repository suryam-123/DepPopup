...
@Injectable()
export class cspfmExecutionCouchDbProvider {
    private db;
    public keysToSchema = {};
    ...
    constructor(private appUtilityObj: appUtility, private network: Network,
        private appPreferences: AppPreferences,private httpClient:HttpClient,
        public formulaCouchDbProvider: formulaCouchDbProvider,
        public observableListenerUtils: cspfmObservableListenerUtils,public executionPouchDbConfiguration: cspfmExecutionPouchDbConfiguration) {
        this.initializePouchDb();
    }

    networkWithConnectivity() {
       // if (this.executionPouchDbConfiguration.configuration.couchDBSyncEnabledObjectSelectors.length > 0) {
            if (this.dbChanges) {
  ...
  *****************************************************/
    ...
    public httpCallSearchDocument(searchText) {
        ...
        return new Promise(resolve => {
            ...
            const url = this.executionPouchDbConfiguration.remoteDbUrl + this.
            executionPouchDbConfiguration.configuration.databaseName + "_search/_design/gettitlewisedocs/_view/ttitlewisedocs";
   ...
   *****************************************************/
    public startChangeListner(last_seq) {
        //const finalSelector = { '$or': this.executionPouchDbConfiguration.configuration.couchDBSyncEnabledObjectSelectors };
        this.dbChanges = this.db.changes({ 
            ...
            .on('error', (err) => {
                console.log("execution couchdb change listener error :", err);
                this.appUtilityObj.couchListenerStopped('executionCouchdb', this)
            });
    }
    ...
    private onDatabaseChange = (change) => {
        ...
        const publishLayaoutIds = this.appUtilityObj.getEventSubscriptionlayoutIds("CouchDB", parsedId.type)
   ...
   *****************************************************/
    ...
    queryHeaderData(headerObjectReferenceDetails, keys, headerDataMapping, dataArray) {
       const uniqueKeys =  lodash.uniq(keys);
        ...
        return this.executeAllDocs(queryOptions, headerObjectReferenceDetails).then(res => {
            const objectType = this.getObjectType(headerObjectReferenceDetails);
            res.forEach(element => {
    ...
    fetchWorkflowValue(viewDocName, keysArray)
    {
        return this.fetchMethod(viewDocName ,keysArray)
    }
    ...
    callSearchDesignDocs(query, designDocName, response?) {
        ...
        const headerstring = this.appUtilityObj.addCredentialforMobile('AJAX', this.executionPouchDbConfiguration)
        return new Promise(resolve => {
            const url = this.executionPouchDbConfiguration.remoteDbUrl + this.executionPouchDbConfiguration.configuration.databaseName
    ...
    //     let objectList = Object.keys(this.executionPouchDbConfigurationDetail.tableStructure);
    //     if (objectList.indexOf(parent_type) < 0) {
    //         this.response = { status: this.failed, message: "Invalid parent type", records: [] }
    //         return Promise.resolve(this.response)
    //     }
    ...
    prepareBulkDoc(type, docs) {
        ...
        docs.forEach(element => {
            const copieddoc = JSON.parse(JSON.stringify(element));
            const updatedDoc = this.setBulkDocDefaultValues(copieddoc, type);
            const reldoc = {};
    ...
    // Set default info in doc
    private setBulkDocDefaultValues(doc, type) {
        const date = new Date();
        const timestamp = date.getTime();
        doc.createdby = this.appUtilityObj.userId;
        doc.createdon = timestamp;
