 ...
 */
export class cspfmSwitchEditor implements Editor {
    ...
    constructor(protected readonly args: EditorArguments) {
        if (!args) {
            throw new Error(
                '[Slickgrid-Universal] Something is wrong with this grid, an Editor must always have valid arguments.'
            );
        }
        this.grid = args.grid;
    ...
    init(): void {
        ...
        //     `${toSentenceCase(columnId + '')} Checkbox Editor`
        // );
        ...
        if (this.hasAutoCommitEdit) {
            ...
            // this._bindEventService.bind(cellContainer, 'click', () => {
            //     this.cellClicked()
            // });
        }
        // this.click()
    ...
    disable(isDisabled = true) {
        ...
        if (this._input) {
            if (isDisabled) {
                ...
                if (prevIsDisabled !== isDisabled && currentValue !== false) {
    ...
    setValue(
        ...
        if (isApplyingValue) {
            ...
            //   this.handleChangeOnCompositeEditor(
            //     null,
    ...
    reset(
        value?: boolean,
        triggerCompositeEventWhenExist = true,
        clearByDisableCommand = false
    ) {
    ...
    validate(_targetElm?: any, inputValue?: any) {
        const isRequired = false ? false : this.columnEditor.required;
        const isChecked =
            inputValue !== undefined ? inputValue : this._input?.checked;
