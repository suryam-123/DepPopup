/* 
 *   File: cspfmBalloonComponent.module.ts 
 *   Copyright(c) 2022 Chain-Sys Corporation Inc.
 *   Duplication or distribution of this code in part or in whole by any media
 *   without the express written permission of Chain-Sys Corporation or its agents is
 *   strictly prohibited.
 */

import {
  NgModule,
  CUSTOM_ELEMENTS_SCHEMA
} from '@angular/core';
import {
  CommonModule
} from '@angular/common';
import { cspfmBalloonComponent } from './cspfmBalloonComponent';
import { FormsModule } from '@angular/forms';
import { cspfmamdModule } from 'src/app/cspfmamd.module';
import { SharedModule } from 'src/core/utils/shared.module';
import { cspfmBalloonDirective } from 'src/core/directive/cspfmBalloon.directive';
@NgModule({
  declarations: [cspfmBalloonComponent, cspfmBalloonDirective],
  imports: [
    CommonModule,
    FormsModule,
    cspfmamdModule,
    SharedModule,
  ],
  exports: [
      cspfmBalloonComponent
  ],
  schemas: [CUSTOM_ELEMENTS_SCHEMA]
})
export class cspfmBalloonComponentmodule {

}