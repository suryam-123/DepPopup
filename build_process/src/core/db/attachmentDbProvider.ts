...
@Injectable()
export class attachmentDbProvider {
    private db;
    private dbChanges;
    ...
    constructor(private appUtilityObj: appUtility, public initialProcess: initialSyncProcess, private appconfig: appConfiguration,
        public observableListenerUtils: cspfmObservableListenerUtils, public attachmentDbConfigurationObj: attachmentDbConfiguration,private httpClient:HttpClient) {
    ...
    *****************************************************/
    ...
    initializePouchDb() {
        if (this.db === undefined) {
            this.remote = this.attachmentDbConfigurationObj.remoteDbUrl + this.attachmentDbConfigurationObj.configuration.databaseName;
            const localdatabseName = this.attachmentDbConfigurationObj.configuration.databaseName + '_' + this.appUtilityObj.orgId + '_' +
    ...
    private setSchema() {
        this.db.setSchema(this.attachmentDbConfigurationObj.configuration.schema);
        this.attachmentDbConfigurationObj.configuration.schema.forEach(type => {
            this.keysToSchema[type.singular] = type;
        });
        // Table structure for initialize object
        this.tableStructure = this.attachmentDbConfigurationObj.configuration.tableStructure;
    }
    ...
    *****************************************************/
    ...
    oneTimeReplicationFromServer(oneTimeSyncSelector) {
        return this.initialProcess.startProcess(this.db, this.remote, this.attachmentDbConfigurationObj, oneTimeSyncSelector).then(res => {
            if (res) {
    ...
    scheduledReplicationFromServerWithSelector(selector){
    ...
    liveSyncWithSelector(selector) {
        ...
        /* const sequenceId = appConstant.attachmentPouchLatestSeqence
        this.db.get(sequenceId).then(doc => {
            if (doc['last_seq']) {
                ...
                const options = {
                    ...
                    selector: selector,
                    since: doc['last_seq'],
                auth: {"username": window.atob(this.attachmentDbConfigurationObj["credentials"]).split(":")[0], "password": window.atob(this.attachmentDbConfigurationObj["credentials"]).split(":")[1]}
                };
                this.liveReplicationDbObject = this.db.sync(this.remote, options);
            } else {
                const options = {
    ...
    *****************************************************/
    ...
    private httpCallUsingUpdateDesignDoc(path, id, query, rev, userid) {
        const headerstring = this.appUtilityObj.addCredentialforMobile('AJAX', this.attachmentDbConfigurationObj)
    ...
    *****************************************************/
    ...
    fetchChildDocsWithRelationshipByParentTypeAndId(child_type, parent_type, parent_id, withchild: boolean, referencedetail?) {
        ...
        const objectList = Object.keys(this.attachmentDbConfigurationObj.configuration.tableStructure);
        ...
        if (validateStatus.status === this.success) {
            if (withchild) {
                return this.db.rel.find(type, id).then(res => {
                    return this.checkChildObjectsForFetch(type, res, referencedetail).then(childPbjectresponse => {
                        return this.checkLookupObjectsForFetch(type, childPbjectresponse, referencedetail).then(result => {
                            this.response = { status: this.success, message: '', records: result };
                            return this.response;
                        });
                    });
                ...
                return this.db.rel.find(type + '_only', id).then(restype => {
                    return this.checkLookupObjectsForFetch(type, restype, referencedetail).then(result => {
                        this.response = { status: this.success, message: '', records: result };
                        return this.response;
                    });
                }).catch(error => {
    ...
    *****************************************************/
    ...
    oneTimeReplicationFromServerWithSpecificObject(selector) {
        return this.initialProcess.fetchAndInsertBulkDocsWithSpecificObjects(this.db, this.remote, this.attachmentDbConfigurationObj, selector).then(initialProcessRes => {
            if (initialProcessRes) {
                return Promise.resolve('Success');
            } else {
                return Promise.resolve('Failed');
            }
    ...
    setCurrentObjectSetInLocalStorage() {
        ...
        this.attachmentDbConfigurationObj.configuration.pouchDBSyncEnabledObjectSelectors.forEach(element => {
            if (element['data.type']) {
                syncEnabledObjectSelectors.push(element['data.type']);
            }
        });
        this.getDocumentFromLocalStorage(appConstant.syncEnabledAttachmentObjectDocName).then(document => {
            let doc;
            if (document) {
                doc = {
                    _id: appConstant.syncEnabledAttachmentObjectDocName,
                    _rev: document._rev,
