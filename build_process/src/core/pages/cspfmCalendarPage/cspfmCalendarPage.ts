import { Component, ChangeDetectionStrategy, OnInit, ChangeDetectorRef, Injectable, HostListener, QueryList, ViewChild, ViewChildren } from '@angular/core';
...
export class cspfmCalendarPage implements OnInit {
  ...
  public redirectUrl = "/menu/cspfmCalendarPage";
  public isballoonEnable = false;
  public info: any;
  ...
  closePopover() {
    ...
    $('cspfmballooncomponent').fadeIn("slow", "linear")
    $('cspfmballooncomponent').parents('.mde-popover-panel').remove();
    this.trigger.closePopover()
  }

  showBalloonLayoutOnHover(event: any, eventType: any, mouseEvent) {
    if (event['meta']['eventLayoutInfo']['isBalloonEvent'] && event['meta']['eventLayoutInfo']['BalloonEventAction'] === 'hover') {
      this.balloonEventType = eventType;
      this.closePopover();
      this.trigger.closePopover();
      this.trigger.openPopover();
      $(document).find(".mde-popover-content").addClass('cs-hide-before-popup-opens');
      this.balloonClickPosition(mouseEvent);

      this.appUtilityObject.dynamicImport({ baseType: 'layout', fileName: event['meta']['eventLayoutInfo']['ViewEventBalloonLayoutName'] }).then((layoutname) => {
        const balloonconfiguredfieldlist = {
          'layoutName': layoutname,
          'id': event.id
        }
        const selectedFieldConfigure = balloonconfiguredfieldlist;
        selectedFieldConfigure['redirectUrlForNav'] = this.isPopUpEnabled ? this.redirectUrl : "/menu/cspfmCalendarPage";
  ...
  balloonClickPosition(event) {
    setTimeout(() => {
      ...
      $(document).find('.cs-hide-before-popup-opens').removeClass('cs-hide-before-popup-opens');
      let _ballHeight = $("cspfmballooncomponent").find(".cs-balloon-popup-style").height()
      let currentEventTarget = event['target'];
      $('.cs-balloon-for-calender').removeClass('active');
      if (currentEventTarget.classList.contains('cs-balloon-for-calender')) {
        $(currentEventTarget).addClass('active');
      } else {
        $(currentEventTarget).parents('.cs-balloon-for-calender').addClass('active');
      }
      var ballBtnleft = $('.cs-balloon-for-calender.active').offset()['left']
      var ballBtnWidth = $('.cs-balloon-for-calender.active').width()
      var ballPopupWidth = $("cspfmballooncomponent").find('.cs-balloon-popup-style').width() || 352;// max-width for popup > 352
      let ballCellHeight = $('.cs-balloon-for-calender.active').height()
      let cellPos = currentEventTarget.getBoundingClientRect();

      $('cspfmballooncomponent').parents('.cdk-overlay-connected-position-bounding-box').css('position', 'unset')
      $('cspfmballooncomponent').parents('.cdk-overlay-pane').css('position', 'absolute');
      let cellPosTop = cellPos.top > 50 ? cellPos.top : 50;

      if ((window.innerWidth - cellPos.left) - (ballPopupWidth + ballBtnWidth) > 0) {
        console.log("Popup right side == arrow to left");
        $('cspfmballooncomponent').parents('.cdk-overlay-pane').css('left', cellPos.left + ballBtnWidth); // 15 for arrow
      } else {
        console.log("Popup left side == arrow to right");
        $('cspfmballooncomponent').parents('.cdk-overlay-pane').css('left', cellPos.left - ballPopupWidth - 22); // 15 for arrow
      }

      // choose top or bottom or center
      if (windowHeight - (cellPosTop + _ballHeight) < 0) {
        console.log("BOTTOM to Top")
        // find hidden area of popup height and add with ct
        $('cspfmballooncomponent').parents('.cdk-overlay-pane').css("top", (cellPosTop - 25) - ((cellPosTop + _ballHeight) - windowHeight));
      } else {
        console.log("Top to BOTTOM")
        $('cspfmballooncomponent').parents('.cdk-overlay-pane').css("top", cellPosTop - 10);
      }
  ...
  showBalloonLayoutOnClick(event: any, eventType: any, mouseEvent) {
    if (event['meta']['eventLayoutInfo']['isBalloonEvent'] && event['meta']['eventLayoutInfo']['BalloonEventAction'] === 'click') {

      this.balloonEventType = eventType;
      this.closePopover();
      this.trigger.openPopover();
      $(document).find(".mde-popover-panel").addClass('cs-hide-before-popup-opens');
      this.balloonClickPosition(mouseEvent);
      this.appUtilityObject.dynamicImport({ baseType: 'layout', fileName: event['meta']['eventLayoutInfo']['ViewEventBalloonLayoutName'] }).then((layoutname) => {
        const balloonconfiguredfieldlist = {
          'layoutName': layoutname,
  ...
  async handleEvent(action: string, event: CalendarEvent, datedisplay?: any, positionevent?: any, itemsfrompopover?: any, eventType?) {
    console.log(action, datedisplay)
    let actionType, enablePopUp, navigationlayout;

    // if(event['meta']['eventLayoutInfo']['isViewEvent']){
    if (event['meta']['eventLayoutInfo']['isBalloonEvent'] && event['meta']['eventLayoutInfo']['BalloonEventAction'] === 'click') {
      // this.showBalloonLayoutOnClick(event.id, event, positionevent,itemsfrompopover)
      this.showBalloonLayoutOnClick(event, eventType, positionevent)
      return
    } else if (event['meta']['eventLayoutInfo']['isViewEvent']) {
      actionType = 'View'
      enablePopUp = event['meta']['eventLayoutInfo']['isEventDetailPopupEnable'] ? true : false;
      navigationlayout = event['meta']['eventLayoutInfo']['eventDetailLayoutName']
    }
    else if (event['meta']['eventLayoutInfo']['isEditEvent']) {
      actionType = 'Edit'
      enablePopUp = event['meta']['eventLayoutInfo']['isEditEventPopupEnable'] ? true : false
      navigationlayout = event['meta']['eventLayoutInfo']['editEventLayoutName']
    } else {
      return
    }
    var queryParamsRouting = {
      ...
      navigationInfo: {
        ...
        navigationUrl: navigationlayout,
        redirectUrl: "cspfmCalendarPage"
      },
    ...
    this.decidePopupOrNavigation(queryParamsRouting, event)
  }

  decidePopupOrNavigation(actionInfo, data) {
    let id = actionInfo['id']
    actionInfo['navigationInfo']['enablePopUp'] ? this.dialogOpenComponent(actionInfo, id) : this.appUtilityObject.navigationToComponent(actionInfo, id);
  }

  dialogOpenComponent(actionInfo, id) {
    ...
    dialogConfig.panelClass = 'cs-dialoguecontainer-large'
    this.appUtilityObject['currentPage'] = actionInfo['navigationInfo']['navigationUrl'];
  ...
  async moreButtonPressed(day, event) {
    ...
    let balloonhtmlElement = document.getElementById('cs-dropdown-' + this.layoutId);
    ...
    setTimeout(() => {
      ...
      window.$(".cs-dropdown-open").jqDropdown("attach", ["#cs-dropdown-custom-123"]);
      window.$(".cs-dropdown-open").jqDropdown("show", [".cs-dropdown"]);
    }, 100)
  }
  popUpResponse(item, dataDisplay, positionevents, items) {
  ...
  async addEventForDate(dateVal, event) {
    ...
    let htmlElement: HTMLElement = document.getElementById('cs-dropdown-custom-123');
    htmlElement.innerHTML = ""
    htmlElement = document.getElementById('cs-dropdown-' + this.layoutId);
    if (htmlElement)
      htmlElement.innerHTML = ""
    this.slickgridpopoverservice.appendComponentToElement_View('cs-dropdown-custom-123', popoverpage, {
      setdateDisplay: datedisplay,
